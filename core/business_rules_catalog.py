...
BUSINESS_RULE_TABLES: tuple[BusinessRuleTable, ...] = (
    ...
    BusinessRuleTable(
        ...
        rows=(
            ...
            ("recovery_pay", "money", "סכום ישיר", f"סמל ברירת המחדל הידוע: {RECOVERY_PAY_MERAV_CODE}."),
...
BUSINESS_RULE_SECTIONS: tuple[BusinessRuleSection, ...] = (
        ...
        rules=(
            BusinessRule(
                ...
                details=(
                    ...
                    "עד 04/2026, אם המדריך עבד בדירה ביום החג, או בחלון חג מיוחד כמו יום העצמאות, התשלום לאותו חג מתבטל רק באותה דירה.",
                    "מ-05/2026, אם המדריך עבד ביום/חלון החג בדירה כלשהי, תשלום החג לאותו יום מתבטל עבורו בכל הדירות.",
    ...
    BusinessRuleSection(
        key="recovery-pay",
        title="דמי הבראה",
        description=f"רכיב {RECOVERY_PAY_MERAV_CODE} המשולם בחודש {RECOVERY_PAY_MONTH:02d} לפי ותק ואחוז משרה.",
        rules=(
            BusinessRule(
                title="זכאות בסיסית",
                summary="דמי הבראה משולמים למדריכים עם ותק של שנה ומעלה שיש להם פעילות בחודש יוני.",
                details=(
                    "הכלל חל גם על מדריכים שאינם קבועים או שאינם מסומנים פעילים, אם עברו שנת ותק ויש להם פעילות בחודש התשלום.",
                    "שנת ותק להבראה נספרת לפי גבול 01/07: מי שהתחיל ב-01/07 של השנה הקודמת נחשב עם שנה בתשלום יוני.",
                    "ימי ההבראה מחושבים כממוצע חודשי של מדרגות הוותק בתוך שנת ההבראה 07-06, כך שמעבר מדרגה באמצע השנה נותן חלק יחסי.",
                    "בנוסף נדרשת לפחות משמרת או רכיב תשלום בחודש התשלום עצמו, למשל 06/2026.",
                    "הדגל is_active אינו תנאי; משמרת או רכיב תשלום ביוני הם שקובעים פעילות.",
                    f"תעריף יום הבראה מוגדר כ-{RECOVERY_PAY_DAILY_RATE:.1f} ש\"ח.",
                ),
                source=("core/recovery_pay.py:calculate_recovery_pay_for_person",),
                tags=("הבראה", RECOVERY_PAY_MERAV_CODE),
                effective_from="06/2026",
            ),
            BusinessRule(
                title="אחוז משרה",
                summary=f"אחוז המשרה מחושב לפי שעות זכאות שנתיות חלקי 12 חודשים וחלקי {RECOVERY_PAY_FULL_TIME_MONTHLY_HOURS} שעות למשרה מלאה.",
                details=(
                    "שעות זכאות כוללות עבודה בפועל, חופשה, כל שעות המחלה כולל חלק שלא שולם, ותשלום חג כשעות שקולות.",
                    "פירוקים כמו calc150_shabbat, calc150_shabbat_100 ו-calc150_shabbat_50 אינם נספרים בנפרד כדי לא ליצור כפילות.",
                    "כוננות אינה נכללת כרגע עד קבלת החלטה משפטית מפורשת.",
                    "אחוז המשרה מוגבל לתקרה של 100%.",
                ),
                source=("core/recovery_pay.py:recovery_eligible_minutes_from_totals",),
                tags=("הבראה", "אחוז משרה", "שעות"),
                effective_from="06/2026",
            ),
            BusinessRule(
                title="מקור ישן ל-06/2026",
                summary="ב-06/2026 ארבעת החודשים 07/2025-10/2025 מגיעים מאחוז המשרה שיובא מהמערכת הישנה.",
                details=(
                    "הקובץ הישן משמש רק לאחוז משרה, לא לסכום ולא לקוד השכר.",
                    f"אחוז המשרה מהקובץ מוכפל ב-{RECOVERY_PAY_FULL_TIME_MONTHLY_HOURS} וב-4 חודשים כדי להפוך אותו לשעות שקולות.",
                    "החודשים 11/2025-06/2026 מחושבים מתוך דיור003.",
                ),
                source=("core/recovery_pay.py:parse_legacy_recovery_xlsx", "core/recovery_pay.py:import_legacy_recovery_fte"),
                tags=("הבראה", "ייבוא", "06/2026"),
                effective_from="06/2026",
            ),
        ),
    ),
    BusinessRuleSection(
        key="clothing-pay",
        title="דמי ביגוד",
        description=f"רכיב {CLOTHING_PAY_MERAV_CODE} המשולם בחודש {CLOTHING_PAY_MONTH:02d} למדריכים קבועים בצוהר הלב.",
        rules=(
            BusinessRule(
                title="זכאות בסיסית",
                summary="דמי ביגוד משולמים למדריכים קבועים בצוהר הלב עם שנת ותק ופעילות ביולי.",
                details=(
                    "לשנת 2026, מי שהתחיל עד 01/08/2025 עומד בתנאי הוותק.",
                    "החישוב השנתי הוא עבור אוגוסט בשנה הקודמת עד יולי בשנת התשלום.",
                    "הדגל is_active אינו תנאי; משמרת או רכיב תשלום ביולי הם שקובעים פעילות.",
                ),
                source=("core/clothing_pay.py:calculate_clothing_pay_for_person",),
                tags=("ביגוד", CLOTHING_PAY_MERAV_CODE),
                effective_from="07/2026",
            ),
            BusinessRule(
                title="אחוז משרה וסכום",
                summary=f"הסכום המרבי הוא {CLOTHING_PAY_FULL_TIME_AMOUNT:.0f} ש\"ח, לפי אחוז משרה שנתי ותקרה חודשית של {CLOTHING_PAY_FULL_TIME_MONTHLY_HOURS} שעות.",
                details=(
                    "שעות מזכות זהות להבראה: עבודה, חופשה, כל המחלה ותשלום חג כשעות שקולות; ללא כוננות.",
                    "כל חודש מוגבל בנפרד ל-182 שעות, והמכנה השנתי הוא 2,184 שעות.",
                    "ב-07/2026 החודשים 08-10/2025 מגיעים מקובץ השעות הישן והשאר מדיור003.",
                ),
                source=("core/clothing_pay.py:calculate_clothing_pay_for_person",),
                tags=("ביגוד", "אחוז משרה", "שעות"),
                effective_from="07/2026",
            ),
        ),
    ),
    ...
    BusinessRuleSection(
        ...
        rules=(
            ...
            BusinessRule(
                title="שיוך דיווחים לחודש תשלום מאוחר",
                summary="משמרות ורכיבי תשלום שסומנו לחודש תשלום מאוחר מוצגים בחודש העבודה אך אינם משולמים בו.",
                details=(
                    "הדיווח נשאר גלוי ומסומן 'לתשלום ב-MM/YYYY' במסך, בדוחות, ב-PDF ובייצוא הדוחות.",
                    "המשמרת או רכיב התשלום מוחרגים מחישוב השכר הרגיל ומגשר של חודש העבודה.",
                    "בחודש התשלום הם מחושבים לפי נתוני חודש העבודה ומיוצאים דרך מנגנון ההשלמות המאושר.",
                    "מדריך עם השלמה מאושרת מופיע גם בחיפוש וברשימת הדוחות של חודש התשלום, גם כשאין לו עבודה בחודש זה.",
                    "דוח חודש התשלום מציג את שורות גשר של ההשלמה, כולל הסמל והסכום, והן נכללות בסך לתשלום.",
                    "חישובי השוואת השלמות רשאים לכלול את הפריטים הנדחים במפורש כדי לחשב את מלוא ההפרש פעם אחת.",
                ),
                source=(
                    "core/payment_period.py:filter_items_for_work_month",
                    "core/report_presence.py:get_payment_period_presence_counts",
                    "app_utils.py:get_daily_segments_data",
                    "core/logic.py:calculate_monthly_summary",
                    "routes/guide.py:prepare_guide_pdf_data",
                    "services/gesher_difference.py:build_approved_completion_gesher_rows",
                ),
                tags=("חודש תשלום", "השלמות", "גשר", "דוחות"),
            ),
            BusinessRule(
            ...
            BusinessRule(
                title="סמלי גשר להפרשי השלמות",
                summary="אירועי salary_impact_events מאושרים מחושבים מחדש ומיוצאים בסמלי רטרו ייעודיים.",
                details=(
                    "סכום ההשלמה אינו נשמר; כל מסך וייצוא מחשבים מצב חודשי לפני ואחרי האירועים.",
                    "יצירה מתהפכת להסרת הרשומה, מחיקה להחזרת old_data ועדכון להחלפת new_data ב-old_data.",
                    "רק אירועים בסטטוס included_in_export נכנסים לייצוא; אירועים exported אינם משולמים שוב.",
                    "רשומה שסומנה לחודש תשלום מאוחר ואין לה salary_impact_event מחושבת במנגנון הישן.",
                    "במנגנון הישן, כשיש גשר סופי מיוצא מלוא הפער מול החישוב הנוכחי; כשאין גשר סופי מיוצאת רק השפעת הרשומות המסומנות.",
                    "הבחירה בין המנגנונים היא לפי קיום אירוע תואם ולא לפי חודש תשלום קשיח.",
                    "רכיבי מקור לפנסיה ממופים לסמל 317.",
                    "רכיבי מקור לא לפנסיה ממופים לסמל 253.",
                    "מחלה וחופשה אינן נכנסות לסמלי הכסף אלא לסמלי רטרו ייעודיים: תשלום מחלה לסמל 306 ותשלום חופשה לסמל 332 (שעות בתעריף חודש העבודה), ניצול ימי מחלה לסמל 414 וניצול ימי חופשה לסמל 427, וזכאות חודשית לימי מחלה לסמל 410 ולימי חופשה לסמל 799 (מספר ימים בלבד, ללא תעריף).",
                    "הזכאות החודשית נגזרת ממספר ימי העבודה של חודש העבודה, ולכן כל השלמה שמשנה ימי עבודה מייצרת גם הפרש זכאות, לרוב בשברי יום.",
                    "המיפוי לסמלי הרטרו נקבע לפי המפתח הפנימי ולא לפי סמל המקור, כדי שלא יישבר כששינו סמל מירב במסך סמלי שכר.",
                    "סמל רטרו של שעות משלם תמיד בתעריף של חודש העבודה; אם התעריף ששולם היה שונה, נוצרות שתי שורות - שלילית בתעריף הישן וחיובית בתעריף חודש העבודה.",
                    "כמות ימים שלילית בסמלים 414 ו-427 מיוצאת כרגיל, כדי לקזז ניצול שבוטל רטרו.",
                    "שורות ימים אינן נספרות בסכום הכספי של ההשלמות.",
                    ...
                    "קובץ גשר סופי משמש בעמוד ההשלמות לבקרה בלבד ואינו מקור סכום ההשלמה.",
                    "בקרת גשר לחודש תשלום כוללת רק רשומות לא דחויות ורשומות שסומנו בדיוק לאותו חודש תשלום; רשומות שסומנו לחודש אחר אינן נכללות בהשוואה.",
                    "הבקרה מסננת את העובדים לפי מערך הדיור וקוד החברה של קובץ הגשר כדי למנוע התאמה שגויה של קוד מירב זהה ממערך אחר.",
                    "כל צרכני ההשלמות משתמשים בתוצאה המאוחדת, לרבות ייצוא חברה, מדריך יחיד, דוחות וסיכומים.",
                ),
                source=("services/salary_impact.py:build_salary_impact_completion_rows", "services/gesher_difference.py:build_completion_gesher_rows"),
                tags=("השלמות", "גשר", "רטרו", "חופשה", "מחלה"),
            ),
            BusinessRule(
            ...
            BusinessRule(
                title="עובד שלא הופיע בגשר הסופי בגלל קוד מירב חסר",
                summary="עובד שנעדר לחלוטין מקובץ הגשר המסומן סופי מוצג כמועמד נפרד ודורש אישור כללי לפני גלגול לחודש תשלום.",
                details=(
                    "הנוכחות נבדקת לפי מפעל, מערך דיור וקוד מירב ורק מול הקובץ שמסומן סופי.",
                    "אם העובד הופיע אפילו בשורה אחת בקובץ הסופי, לא נוצר מועמד מהסוג הזה.",
                    "דיווחי השלמה משויכים לפי חודש התשלום ולא לפי חודש הדיווח; השלמה שכבר משויכת לחודש התשלום הנבחר נשארת באזור ההשלמות הרגיל ואינה נספרת בכרטיס העובד החסר.",
                    "האישור מעביר יחד את כל דיווחי חודש העבודה במערך, וכן השלמות מחודשים קודמים שסומנו לחודש הגשר שבו העובד נעדר, למעט פריטים שכבר מסומנים לחודש תשלום עתידי.",
                    "קוד חסר או כפול, סכום שלילי, קובץ סופי שהוחלף וחודש תשלום נעול חוסמים אישור.",
                    "שינוי קוד, סכום או קובץ סופי מחזיר מועמד שטרם יוצא למצב ממתין; יצירת קובץ גשר המכיל את העובד נועלת אותו כנכלל בגשר.",
                ),
                source=(
                    "services/missing_gesher_candidates.py",
                    "routes/completions.py:scan_missing_gesher_candidates",
                    "routes/export.py:_archive_gesher_file",
                ),
                tags=("השלמות", "גשר סופי", "קוד מירב", "אישור"),
            ),
            BusinessRule(
